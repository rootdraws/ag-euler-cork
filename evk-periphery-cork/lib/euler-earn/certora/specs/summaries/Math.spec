methods {
    ...
    function _.exactlyOneZero(uint256 x, uint256 y) internal => cvlExactlyOneZero(x,y) expect bool;
    function _.min(uint256 x, uint256 y) internal => cvlMin(x,y) expect uint256;
}

function cvlExactlyOneZero(uint256 x, uint256 y) returns bool {
    return (x == 0 && y != 0) || (x != 0 && y == 0);
}

function cvlMin(uint256 x, uint256 y) returns uint256 {
    return x < y ? x : y;
}
